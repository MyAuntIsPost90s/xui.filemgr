...
function createWebUploader() {
    ...
    script.id = 'webUploader';
    script.src = '/resource/js/common/webUploader/webuploader.js';
    if ($('#webUploader').length < 1) {
        $('head').append(script);
    }
}
...
var AttachMgrUtil = {
    getFileId: function () {
        return xui.utils.getUuid().toUpperCase().replace(/\-/g, '');
    },
    getBId: function () {
...
function AttachMgr(params) {
    var ajax = parent.IQGatewayClientManager().request;
    var layui = parent.global.layui;
    var layer = layui.layer;
    var $ = typeof params.context != 'undefined' && params.context != 1 ? jQuery : parent.jQuery;

    var baseUploader = null;    //baseUploader
    var bId = params.bId;	//业务id
    var baseUrl = params.baseUrl;	//基路径
    var fileTag = params.fileTag;	//文件标签
    var fileType = params.fileType;	//文件类型
    ...
    //url
    var host = parent.global.contextPath; //host
    var blockCheckUrl = host + '/apps/support/attachmgr/blockCheck';    //分块检查
    var completeUrl = host + '/apps/support/attachmgr/complete';        //合并分块
    var cancelUrl = host + '/apps/support/attachmgr/deleteById';	//取消url
    var insertUrl = host + '/apps/support/attachmgr/create';	//插入url
    var downloadUrl = host + '/apps/support/attachmgr/download';	//下载url
    var listUrl = host + '/apps/support/attachmgr/list4BId';	//获取文件列表url
    var batchdeleteUrl = host + '/apps/support/attachmgr/batchDelete';     //批量删除文件
    var uploadUrl = host + '/apps/support/attachmgr/upload';    //文件上传路径
    ...
    //样式
    var StyleObj = createStyle();

    //边框模板
    var borderTemplet = '<div id="attachmgr" class="layui-tab layui-tab-brief filemgr">'
        + '<ul class="layui-tab-title"><li class="layui-this">待处理</li><li class="li-finished">已完成（<span id="sp-finishedCount">0</span>）</li>'
        + '<div style="float: right;padding: 0 10px;"><button id="btn-selectFile" class="layui-btn layui-btn-small">选择文件</button></div>'
        + '</ul>'
    ...
    var doTemplet = '<table class="layui-table" lay-skin="line" >'
        ...
        + '<div class="btn-menus"><button id="btn-alldel" class="layui-btn layui-btn-small layui-btn-primary">批量删除</button></div>';
    //上传文件项
    var uploadItemTemplet = '<tr id="{0}" data-fileId="{3}" data-fileSize="{4}" data-nowPercentage="0" data-finished="0" data-fileUrl="" data-filePath="" class="upload-item">'
        ...
        + '</div></td><td width="150">'
        + '<button class="layui-btn layui-btn-mini btn-upload">上传</button>'
        + '<button class="layui-btn layui-btn-warm layui-btn-mini btn-cancel">取消</button>';
    //已完成文件项
    var finishedItemTemplet = '<tr id="{0}" data-fileid="{0}" data-fileSize="{3}" class="upload-item">'
        ...
        + '<div class="percent"></div>'
        + '</div></td><td width="150">'
        + '<button class="layui-btn layui-btn-mini btn-download">下载</button>'
        + '<button class="layui-btn layui-btn-warm layui-btn-mini btn-del">删除</button>'
        + '</td></tr>';
    ...
    var loadFinished = function (data) {
    ...
    function createStyle() {
        var html = '#attachmgr .item-info {' +
            'height: auto;' +
            'overflow: hidden;' +
            'zoom: 1;' +
            '}' +
            '#attachmgr .upload-item .tip {' +
            'float: left;' +
            'width: 20%;' +
            '}' +
            '#attachmgr .upload-item .filename {' +
            'float: left;' +
            'width: 35%;' +
            'overflow: hidden;' +
            ...
            'padding: 0 5px' +
            '}' +
            '#attachmgr .upload-item .filesize {' +
            'float: left;' +
            'width: 30%;' +
            'overflow: hidden;' +
            'text-overflow: ellipsis;' +
            ...
            'float: right;' +
            '}' +
            '#btn-selectFile input {' +
            'display: none;' +
            'width: 0px;' +
            'height: 0px;' +
            'float: left;' +
            '}';
        var style = xui.appendStyle(html, 'xuiAttachMgrStyle');

        return style
    }

    /**
     * 封装webuploader,基本上传服务
     * author:caichenghh
     * date:2018/04/20
     */
        ...
        WebUploader.Uploader.register({
            ...
            beforeSend: function (block) {
                ...
                if (typeof(onBeforeSendBlock) == 'function') {
                    var isReject = onBeforeSendBlock(block, this);
                    if (isReject) {
                        deferred.reject();
                    }
                }
...
//删除文件
    function deleteFile(fileId) {   //fileId:文件主键
        //通知服务端删除
        ajax({
            url: cancelUrl,
            data: {fileId: fileId},
...
//绑定事件
    function eventInit() {
        //选择文件点击事件
        $('#btn-selectFile').off('click').click(function () {
        ...
        $('#btn-yes').off('click').click(function () {
            ...
            $('tr[data-finished=1]').each(function () {
                var $tr = $(this);
                result.push({
                    fileName: $tr.find('.filename').text(),
                    fileId: $tr.attr('data-fileId'),
                    filePath: $tr.attr('data-filePath'),
                    fileUrl: $tr.attr('data-fileUrl')
                });
            });
            if (onComplete != null) {
                onComplete(result);
        ...
        //点击取消事件
        $('#btn-allCancel').off('click').click(function () {
            var ids = [];
...
//文件上传前回调
    function onBeforeSendFile(file, md5) {
        ...
        if (fileTag != null) { //文件标签
            postData.fileTag = fileTag;
        }
        if (fileTag != null) { //文件类型
            postData.fileType = fileType;
        }
        ajax({
            url: insertUrl,
            ...
            onSuccess: function (data) {
                ...
                $file.attr('data-filePath', data.data.filePath);
                $file.attr('data-fileUrl', data.data.fileUrl);
            }
        });
    }
...
//分块上传前回调
    function onBeforeSendBlock(block, _this) {
        ...
        ajax({
            ...
            data: postData,
            onSuccess: function (data) {
                if (data.code == 1 && data.msg == 'true') {  //分块存在，跳过,跳过不能显示进度条
                    $tip.html('正在处理');
                    isExist = true;
                }
...
//文件上传结束回调
    function onAfterSendFile(file) {
        ...
        if (completeUrl == null || completeUrl == '') {
            console.log('completeUrl为空，无法合并文件');
            file.setStatus('error');
            return;
        }
        var id = $file.attr('data-fileId');
        var postData = {fileId: id};
        //上传完成，提示后台合并
        ajax({
            url: completeUrl,
            async: false,
    ...
    function onBeforeFileQueued(file) {
        ...
        var html = uploadItemTemplet.format(file.id, file.name, sizeStr, AttachMgrUtil.getFileId(), file.size);
...
//绑定单个文件事件
    function bindFileItemEvent($file) {
        //上传
        $file.find('.btn-upload').off('click').click(function () {
            var id = $file.attr('id');
        ...
        //取消
        $file.find('.btn-cancel').off('click').click(function () {
            var id = $file.attr('id');
            var file = baseUploader.getFile(id);
            baseUploader.cancel(id);
            //通知服务端删除
            deleteFile($file.attr('data-fileId'));
            var url = $file.attr('data-fileUrl');
